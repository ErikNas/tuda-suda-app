...
# Результат: dist/tuda-suda и dist/config.yaml
```

## Конфигурация

`config.yaml` автоматически копируется в `dist/` при сборке. При разработке создайте `config.yaml` в корне проекта (см. пример в репозитории).

### Поиск конфигурации

Приложение ищет `config.yaml` в следующем порядке:
